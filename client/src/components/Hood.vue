  ...
  methods: {
    ...
    update_hood_display: function() {
      for (var i = 0; i < this.n_hypothesis; i++) {
        ...
        var hood_display_elem = document.getElementById('hood_display_' + (i+1))
    ...
    hood_display_3x3: function (i) {
      ...
      for (var col = 0; col < 3; col++) {
        for (var row = 0; row < 3; row++) {
          ...
          buttonLocator.id = 'hood_button_locator_' + i + '_' + position;
          buttonLocator.style.left = col*hood_display_width/3 + 'px';
          buttonLocator.style.top = row*hood_display_height/3 + 'px';
          buttonLocator.style.width = hood_display_width/3 + 'px';
          buttonLocator.style.height = hood_display_height/3 + 'px';

          // color per known symbols
          var button_color = getComputedStyle(document.documentElement).getPropertyValue('--neutral_color');
          var color_name =  this.hood_info.known_symbols_colors[''+position]
          if (color_name == 'flash') {
          ...
          // add to container
          historyContainer.appendChild(buttonLocator)
        }
      }

      //add info in grid
      this.hood_info.symbol_history.forEach( (position, index, array) => {

        var gridElem = document.getElementById('hood_button_locator_' + i + '_' + position)
        gridElem.innerHTML = 'here'
    ...
    hood_text_innerHTML_continuous: function(i) {
      return '' + this.hood_info.hypothesis_probability[i].toFixed(2)
    }
  }
}
